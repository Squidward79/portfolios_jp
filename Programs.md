...
３Dモデルの皮膚や皮膚の中にある骨を見られる機能とモデルを回転しながら見られる機能があります。 
...
#### 説明
  CUDAを利用し、映像について並列で演算を処理するプログラムです。
  運転手目線で高速道路を走行する映像から車線を検出して直線で見せることやガウシアンフィルタのようなフィルタを入れることができるプログラムです。
...
卒業制作の時VIVE PROを使って仮想のMIDI楽器LaunchPadをMR環境で演奏できるプログラムと、スマホがVIVEと同時に合奏できるアプリケーションです。
周辺の空間を認識して現実基盤のMeshを作ります。その後演奏するときに生成されるParticleが現実に相互作用します。Meshの生成はVIVEで提供するAPIを使用しました。
